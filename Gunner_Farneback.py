...
def drawFlow(lines, img,flow,step=16):
    ...
    for x,y in indices:   # 인덱스 순회
        ...
        if distance > 5:
            print(distance)
            optical_flow_array.append([(x,y),(dx,dy)])
            cv2.line(lines, (x, y), (x+dx,y+dy), (0,0,255), 1, cv2.LINE_AA)

            total_x = (total_x + x)
            total_y = (total_y + y)

    if total_x != 0 :
        total_x = int(total_x / len(optical_flow_array))
        total_y = int(total_y / len(optical_flow_array))
    
        cv2.circle(lines, (total_x,total_y), 7, (255,255,255), -1)
        
    return optical_flow_array, lines



roots = 'ball_t1.mp4'
video_name = 'b_001'
output_root = 'Gunner_Farneback_{}.mp4'.format(video_name)
...
# frame = cv2.resize(frame, dsize=(640, 480), interpolation=cv2.INTER_AREA)
  ...
  # 최초 프레임 경우 
    if prev is None: 
        ...
        # 계산 결과 그리기, 선언한 함수 호출 ---⑧
        optical_flow_array,lines = drawFlow(lines,frame,flow)
        # 다음 프레임을 위해 이월 ---⑨
        prev = gray
        for tracking in optical_flow_array:
            print(tracking)
            cv2.circle(frame, tracking[0], 5, (255,0,0), -1)
